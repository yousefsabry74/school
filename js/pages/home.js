...
(async function () {
  ...
  requestAnimationFrame(() => {
    setTimeout(() => {
      ...
      APP.LEVELS.forEach((level, i) => {
        if (xp >= level.xp) $(`ms${i}`)?.classList.add("reached");
      });
    }, 300);
  });
  ...
  function mascotTalk() {
    ...
    speech.textContent = speeches[speechIdx];
    void speech.offsetHeight;
    speech.style.animation = "speech-show .5s ease";
  }
  ...
  if (grid) {
    LESSONS.forEach((lesson, idx) => {
      const done = lesson.sessions_data.filter((sessionItem) =>
        PROGRESS.getQuiz(user.id, lesson.id, sessionItem.id),
      ).length;
      const pct = lesson.sessions
        ? Math.round((done / lesson.sessions) * 100)
        : 0;
      const col = lesson.color;
      const isAvail = lesson.available;
      const isDone = pct === 100;
      const dotWidth = Math.max(20, Math.floor(80 / lesson.sessions));
        ...
        <div class="lesson-card-body">
          ...
          <div class="sessions-dots">
            ${Array.from({ length: lesson.sessions }, (_, i) => {
              const quiz = PROGRESS.getQuiz(user.id, lesson.id, i + 1);
              return `<div class="s-dot ${quiz ? "done" : ""}" style="width:${dotWidth}px;${!quiz ? "background:rgba(255,255,255,0.1)" : ""}"></div>`;
            }).join("")}
          </div>
    ...
    const achs = [
      {
        icon: "⭐",
        name: "أول خطوة",
        desc: "أكمل حصتك الأولى",
        color: "#FFD600",
        ok: completedSessions >= 1,
      },
      {
        icon: "🔥",
        name: "متعلم نشيط",
        desc: "أكمل 3 حصص",
        color: "#FF6B35",
        ok: completedSessions >= 3,
      },
      {
        icon: "🏆",
        name: "بطل الاختبارات",
        desc: "احصل على 80% في اختبار",
        color: "#6C3EFF",
        ok: PROGRESS.hasHighScore(user.id, 80),
      },
      {
        icon: "🌟",
        name: "نجم العلوم",
        desc: "أكمل جميع الحصص",
        color: "#00D97E",
        ok: completedSessions >= totalSessions,
      },
    ];

    achs.forEach((achievement) => {
      achGrid.insertAdjacentHTML(
        "beforeend",
        `
        <div class="ach-card ${achievement.ok ? "unlocked" : "locked-ach"}">
          <div class="ach-icon-wrap" style="background:${achievement.color}22;border:1px solid ${achievement.color}44;">
            <span style="font-size:22px">${achievement.icon}</span>
          </div>
          <div>
            <div class="ach-name">${achievement.name}</div>
            <div class="ach-desc">${achievement.desc} ${achievement.ok ? "✅" : ""}</div>
          </div>
        </div>`,
      );
    });
